...
const ProductFavoriteButton = ({ cln, id, handleAddToWishlist }) => {
  const wishlist = useSelector((state) => state.wishlist.wishlist);
  const isItemInWishlist = wishlist.find((item) => item.id === id);
  const dispatch = useDispatch();

  const [isSelected, setIsSelected] = useState(isItemInWishlist);
  ...
  return (
    <ShadowButton onClick={handleAddProductToWishlist} cln={cln ? cln : ""}>
      {isSelected ? (
        <svg
          width="30px"
          height="30px"
